...
export default function Accordian () {
  ...
  function handleSigleSelection (getCurrentId) {
    ...
    function handleMultiSelection(getCurrentId) {
        ...
        if(findIndexOfCurrentId === -1)copyMultiple.push(getcurrent)
    }
  return (
        ...
        {data && data.length > 0 ? (
          data.map(dataItem => (
            <div key={dataItem.id} className='item'>
              <div
                onClick={enableMultipleSelection? handleMultiSelection(dataItem.id() ) :() => handleSigleSelection(dataItem.id)} //we check to see if the multiselction mode is enabled and use its funtion otherwise we use the single selection mode
                className='title'
              >
